...
import { Loader2, Edit2, Trash2, Filter, AlertCircle, CheckCircle, Clock, XCircle } from "lucide-react";
...
export default function AdminLoans() {
  ...
  const updateLoan = trpc.loans.updateStatus.useMutation();
  ...
  const handleUpdateStatus = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (loan && previousStatus !== editData.status) {
        ...
        const notificationMessages: Record<string, string> = {
          approved: `Sua solicitação de empréstimo para ${loan.institution} foi aprovada!`,
          rejected: `Sua solicitação de empréstimo para ${loan.institution} foi rejeitada.`,
          returned: `O empréstimo para ${loan.institution} foi marcado como devolvido.`,
        };

        if (notificationTitles[editData.status]) {
          await createNotification.mutateAsync({
            userId: loan.userId,
            type: `loan_${editData.status}` as any,
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-foreground">Gerenciar Empréstimos de Kits</h2>
        <p className="mt-1 text-muted-foreground">
          {loans.length} solicitação{loans.length !== 1 ? "s" : ""} de empréstimo
        </p>
      </div>
      ...
      {loansLoading ? (
        ...
        <div className="space-y-4">
          {loans.map((loan) => {
            const experiment = experiments.find((e) => e.id === loan.experimentId);
            return (
              <Card key={loan.id} className={`border ${getStatusColor(loan.status)}`}>
                <CardHeader>
                  <div className="space-y-4">
                    <div className="flex items-start justify-between">
                      <div>
                        <CardTitle className="text-foreground">{loan.name}</CardTitle>
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      <div>
                        <p className="text-xs font-medium text-muted-foreground">Experimento</p>
                        <p className="text-sm text-foreground">{experiment?.title || "Experimento não encontrado"}</p>
                      </div>
                      <div>
                        <p className="text-xs font-medium text-muted-foreground">Datas</p>
                        <p className="text-sm text-foreground">
                          {new Date(loan.withdrawalDate).toLocaleDateString("pt-BR")} até{" "}
                    ...
                    <div className="flex gap-2">
                      <Dialog open={editOpen && selectedLoanId === loan.id} onOpenChange={setEditOpen}>
                        ...
                        <DialogContent className="border-border bg-card">
                          <DialogHeader style={{marginTop: '-61px'}}>
                            <DialogTitle className="text-foreground">Atualizar Empréstimo</DialogTitle>
                            <DialogDescription className="text-muted-foreground">
                              {loan.name} - {experiment?.title}
                            </DialogDescription>
                          </DialogHeader>
